...
public class ExplainServiceImpl implements ExplainService {
    ...
    public String smartAdviseForIndex(String sql) {
        ...
        if (!HandleNotSym(sql).equals("")) {
            advise.add(HandleNotSym(sql));
        }

        if (!HandleReverseTable(sql).equals("")) {
            advise.add(HandleReverseTable(sql));
        }
    ...
    private String HandleNotSym(String sql) {
        String[] result = sql.split(" ");
    ...
    private String HandleReverseTable(String sql) {
        String[] result = sql.split(" ");

        for (int i = 0; i < result.length; i++) {
            if (result[i].toLowerCase().equals("select") || result[i+1].equals("*")) {
                return "Select *  可能会产生回表问题";
            }
        }
